    ...
    public static void flush(Context context) {
        ...
        try {
            ...
            for (int i = 0; i < a.length(); i++) {
                ...
                if (item == null || !syncNoQueue(item)) keep.put(item);
            }
            p.edit().putString(KEY, keep.toString()).apply();
        } catch (Exception ignored) {}
    }

    private static boolean syncNoQueue(JSONObject payload) {
        if (BuildConfig.SUPABASE_URL.isEmpty() || BuildConfig.SUPABASE_ANON_KEY.isEmpty() || BuildConfig.SUPABASE_ACCESS_TOKEN.isEmpty()) return false;
        try {
            URL url = new URL(BuildConfig.SUPABASE_URL + "/functions/v1/sofia-lead-sync");
            HttpURLConnection c = (HttpURLConnection) url.openConnection();
