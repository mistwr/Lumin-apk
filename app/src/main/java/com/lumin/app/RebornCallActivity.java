...
public class RebornCallActivity extends AppCompatActivity implements RebornCentral.Listener {
    ...
    public static void refreshFromService() {
        ...
        if (a == null || a.refreshPending) return;
        a.refreshPending = true;
        a.ui.postDelayed(() -> {
            a.refreshPending = false;
            if (current == a && !a.isFinishing()) a.refresh();
        }, 120L);
    }
    ...
    @Override public void onCentralChanged() { refreshFromService(); }
    ...
    private void buildUi() {
        ...
        pipeline = t("Áudio: IDLE · STT: IDLE", 13, Color.rgb(255,198,94), false); pipeline.setGravity(Gravity.CENTER); root.addView(pipeline);
        ...
        Button answer = b("Atender"); answer.setOnClickListener(v -> { RebornInCallService s = RebornInCallService.get(); if (s != null) s.answer(); });
        Button end = b("Desligar"); end.setOnClickListener(v -> { RebornInCallService s = RebornInCallService.get(); if (s != null) s.hangup(); finish(); });
        controls1.addView(answer, weight()); controls1.addView(end, weight()); root.addView(controls1);

        LinearLayout controls2 = row();
        Button mute = b("Mute"); mute.setOnClickListener(v -> { muted = !muted; RebornInCallService s = RebornInCallService.get(); if (s != null) s.setMutedCompat(muted); mute.setText(muted ? "Mute ✓" : "Mute"); });
        Button spk = b("Altifalante"); spk.setOnClickListener(v -> { speaker = !speaker; RebornInCallService s = RebornInCallService.get(); if (s != null) s.setSpeaker(speaker); spk.setText(speaker ? "Altifalante ✓" : "Altifalante"); });
        controls2.addView(mute, weight()); controls2.addView(spk, weight()); root.addView(controls2);

        LinearLayout controls3 = row();
        Button hold = b("Em espera"); hold.setOnClickListener(v -> { RebornInCallService s = RebornInCallService.get(); if (s != null) s.toggleHold(); });
        Button intro = b("Apresentar REBORN"); intro.setOnClickListener(v -> RebornCentral.queueIntro());
        controls3.addView(hold, weight()); controls3.addView(intro, weight()); root.addView(controls3);

        TextView routeTitle = t("TESTE DE SAÍDA DE VOZ", 12, Color.rgb(148,160,194), true); routeTitle.setPadding(0, dp(20), 0, dp(8)); root.addView(routeTitle);
        LinearLayout route1 = row();
        Button auto = b("AUTO"); auto.setOnClickListener(v -> setVoiceRoute(RebornVoiceController.ROUTE_AUTO));
        Button samsung = b("Samsung"); samsung.setOnClickListener(v -> setVoiceRoute(RebornVoiceController.ROUTE_SAMSUNG));
        route1.addView(auto, weight()); route1.addView(samsung, weight()); root.addView(route1);
        LinearLayout route2 = row();
        Button acoustic = b("Acústica"); acoustic.setOnClickListener(v -> setVoiceRoute(RebornVoiceController.ROUTE_ACOUSTIC));
        Button digitalButton = b("Digital EXP"); digitalButton.setOnClickListener(v -> setVoiceRoute(RebornVoiceController.ROUTE_DIGITAL));
        route2.addView(acoustic, weight()); route2.addView(digitalButton, weight()); root.addView(route2);
        ...
        Button audioProbe = b("ANALISAR AUDIO STACK DURANTE A CHAMADA");
        LinearLayout.LayoutParams probeP = new LinearLayout.LayoutParams(-1, dp(62)); probeP.topMargin = dp(10); audioProbe.setLayoutParams(probeP);
        audioProbe.setOnClickListener(v -> {
            Call c = RebornInCallService.activeCall();
            if (c == null || c.getState() != Call.STATE_ACTIVE) {
                Toast.makeText(this, "Faz a análise com a chamada ativa", Toast.LENGTH_SHORT).show();
                return;
            }
            if (RebornAudioRouteProbe.isRunning()) {
                Toast.makeText(this, "Análise já está em curso", Toast.LENGTH_SHORT).show();
                return;
            }
            RebornAudioRouteProbe.start(this);
            refreshFromService();
        });
        root.addView(audioProbe);

        TextView trTitle = t("TRANSCRIÇÃO AO VIVO", 12, Color.rgb(148,160,194), true); trTitle.setPadding(0, dp(24), 0, dp(8)); root.addView(trTitle);
        transcript = t("À espera de voz…", 15, Color.rgb(225,230,242), false); transcript.setPadding(dp(14), dp(14), dp(14), dp(14)); transcript.setBackgroundColor(Color.rgb(14,22,39)); root.addView(transcript, new LinearLayout.LayoutParams(-1, -2));
        TextView rpTitle = t("RESPOSTA REBORN", 12, Color.rgb(148,160,194), true); rpTitle.setPadding(0, dp(20), 0, dp(8)); root.addView(rpTitle);
    ...
    private void setVoiceRoute(String route) { RebornVoiceVerifier.cancel(); RebornVoiceController.setRoute(this, route); refreshFromService(); }

    private void refresh() {
        Call c = RebornInCallService.activeCall();
        if (c == null) state.setText("Sem chamada ativa");
        else {
            try { if (c.getDetails() != null && c.getDetails().getHandle() != null) { String p = c.getDetails().getHandle().getSchemeSpecificPart(); if (p != null && !p.isEmpty()) number.setText(p); } } catch (Throwable ignored) {}
            state.setText(label(c.getState()));
        }
        String mode = getSharedPreferences("sofia_control", MODE_PRIVATE).getString("mode", "AUTO");
        ai.setText("Qwen3 · " + LocalRebornEngine.backendName() + " · " + mode + " · " + RebornCentral.stage() + (RebornCentral.lastLatencyMs() > 0 ? " · " + RebornCentral.lastLatencyMs() + " ms" : ""));
        ...
        pipeline.setText("Áudio: " + RebornCallAudioController.state() + " / PCM " + pcm + " · STT: " + RebornTranscriptionService.state() + " / " + sttInput);
        String tr = RebornCentral.transcript();
        String partial = getSharedPreferences("reborn_central", MODE_PRIVATE).getString("stt_partial", "");
        transcript.setText((tr == null || tr.trim().isEmpty()) ? (partial == null || partial.trim().isEmpty() ? "À espera de voz…" : "… " + partial.trim()) : tr.trim() + (partial == null || partial.trim().isEmpty() ? "" : "\n… " + partial.trim()));
        String r = RebornCentral.lastReply(); reply.setText(r == null || r.trim().isEmpty() ? "A aguardar cliente…" : r);
    ...
    private LinearLayout.LayoutParams weight() { LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(0, dp(62), 1f); p.setMargins(dp(5), 0, dp(5), 0); return p; }
    private Button b(String s) { Button b = new Button(this); b.setText(s); b.setTextSize(14); b.setAllCaps(false); return b; }
